...
TABLE1;
    ...
    private function renderAction(string $content, string $headerData = ''): string {
        $methodKey = $this->rep->currentAction[0];
    ...
    public function magic(): string {
        $method = $this->rep->currentAction[0];
        switch ($method) {
            case 'groupHeader':
            case 'groupFooter':
                return $this->$method(
    ...
    private function getMethodName(): string {
        if (isset($this->rep->currentAction[3])) {
            // direct prototype call from report class
            [$key, $typ, $action, $ownerAction, $ownerTyp] = $this->rep->currentAction;
            $action = $ownerAction;
            $typ = $ownerTyp;
        } else {
            // prototype call from owner class via $report->prototype()
            [$key, $typ, $action] = $this->rep->currentAction;
        }
        switch ($typ) {
